  ...
  return (
        <main className="max-w-screen overflow-x-hidden">
          <section className="relative flex flex-col lg:flex-row gap-8 justify-center items-center p-4 py-32 text-white z-20">
            <Image width={1080} height={1000} src={'/img/mikolajoweRozdanie/' + pageData.photo[0]} alt="Akcja Mali Czarodzieje" className='absolute w-full h-full -z-10 object-cover top-0 left-0'/>
            ...
            <div className="flex flex-col gap-2 justify-start items-start lg:max-w-[50vw]">
              ...
              <h1 className="text-5xl lg:text-8xl font-bold pb-8">{pageData.title}</h1>
              <p className="text-xl lg:text-2xl lg:w-2/3">{pageData.shortDescription}</p>
          ...
          <section className="flex flex-col justify-center items-center p-4 mx-auto">
            <h3 className="text-5xl font-black py-12">Zobacz zdjęcia z akcji "<span className='text-primary'>{pageData.title}</span>"</h3>
            <ul className='columns-1 lg:columns-3 px-4 lg:px-16'>
              {
                pageData.photo.map((photo) => (
                  <li className='p-4' key={photo}>
                    <img src={'/img/mikolajoweRozdanie/' + photo} alt="Zdjęcie z akcji Mali Czarodzieje" />
                  </li>
                ))
              }
            </ul>
