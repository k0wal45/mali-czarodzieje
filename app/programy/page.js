    ...
    <main className="max-w-screen overflow-x-hidden">
      <section className="flex flex-col lg:flex-row gap-8 justify-center items-center p-4 py-32 bg-program text-white">
        <div className="flex flex-col gap-8 justify-center items-start lg:max-w-[50vw]">
          <h1 className="text-5xl lg:text-8xl font-bold">Zorganizowane przez nas akcje</h1>
      ...
      <section className="flex justify-center items-center flex-col gap-8 p-4">
        <h2 className="text-5xl font-black py-24">Możesz pomóc dzieciom wspomagając naszą <span className="text-primary">fundacje</span></h2>
        <ul className="flex flex-col gap-12 w-full items-center justify-center">
          {
            Data.map((event) => (
              <li className="flex flex-col lg:flex-row border-2 border-neutral-300 w-full lg:w-3/4">
                <Image width={700} height={400} src={'/img/mikolajoweRozdanie/' + event.photo[0]} alt={`${event.title} Mali Czarodzieje`} className='h-[25rem] aspect-square object-cover'/>
                <div className="flex flex-col w-full justify-around items-center text-center p-4 gap-4">
                  ...
                  <Link href={`/programy/${event.slug}`} className='my-4 p-4 rounded-sm border-4 border-primary text-black text-xl uppercase text-center w-full lg:w-64'><span>czytaj wiecej</span></Link>
