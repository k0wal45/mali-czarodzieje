...
const ProgramCard = ({photo, title, goal, form, desc}) => {
  return (
    <li className="flex flex-col rounded-xl shadow-xl w-[30rem]" key={title}>
      <img src={photo} alt="" className="aspect-[3/2] object-cover rounded-lg" />
      <div className="flex flex-col p-4 gap-2">
        <h3 className="text-3xl font-lato-black py-4">{title}</h3>
        ...
        <ul className="flex gap-4 flex-wrap max-w-full pt-2">
          {
            form.map((item) => (
              <li key={item} className="rounded-full px-4 py-[3px] bg-primary text-white font-lato-bold">{item}</li>
            ))
          }
        </ul>
